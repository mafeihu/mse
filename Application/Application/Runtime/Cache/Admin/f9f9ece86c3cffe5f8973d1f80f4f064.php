...
function getnums(){
    ...
    window.location.href="/Admin/User/teacher_list?nums="+num+"&start="+start+"&end="+end+"&inner_name="+inner_name;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/User/techer_add">添加</a></span></div>
...
<!--<input name="dele" type="submit" value="删除" onclick="del();" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>-->
   ...
   &nbsp;&nbsp;注册日期： <input type="text" class="laydate-icon" name="start_time" id="start" size="12" value="<?php echo ($start); ?>" readonly> - <input type="text" class="laydate-icon" name="end_time" id="end" size="12" value="<?php echo ($end); ?>" readonly>
        关键词: <input type="text" name="inner_name" id="inner_name" value="<?php echo ($inner_name); ?>" placeholder="输入昵称" size="30">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["inner_teacher_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>

    <td align="center" class="onerow"><img src="<?php echo ($l["inner_img"]); ?>" style="width: 50px;height: 50px;border-radius:50%"></td>
    <td align="center" class="onerow"><?php echo ($l["inner_name"]); ?></td>
  ...
  <td align="center">
                  <a href="<?php echo U('techer_add',array('inner_teacher_id'=>$l['inner_teacher_id']));?>">编辑</a> | <a href="javascript:;"  onclick="del(<?php echo ($l["inner_teacher_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('techer_del');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('teacher_list');?>");
        }else{
            alert('删除失败！');
        }
    });
