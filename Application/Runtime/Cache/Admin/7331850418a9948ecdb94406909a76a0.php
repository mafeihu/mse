...
function edit(id){
	location.href="/mse/Admin/Base/edit/id/"+id;
}
//进入到下级
function xj(id){
	location.href="/mse/Admin/Base/index2/id/"+id;
}
function getnums(){
    var num = $("#nus").val();
    window.location.href="/mse/Admin/Base/gift_list?nums="+num;
}
...
<div class="tools"> 
<div class="add"><span><a href="/mse/Admin/Base/toadd_gift">添加</a></span></div>
...
<tr class="tabTitleMain">
    ...
    <td width="12%" align="center">是否连击</td>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["gift_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    ...
    <td align="center" class="onerow"><?php if( $l["is_special"] == -1 ): ?>非特效<?php elseif( $l["is_special"] == 1 ): ?><span style="color:red;">拳击手套</span><?php elseif( $l["is_special"] == 2 ): ?><span style="color:red;">建筑十书</span><?php elseif( $l["is_special"] == 3 ): ?><span style="color:red;">流水别墅</span><?php elseif( $l["is_special"] == 4 ): ?><span style="color:red;">密斯椅</span><?php endif; ?></td>
